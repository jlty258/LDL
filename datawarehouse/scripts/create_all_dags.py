...
for i, (dag_id, description, layer) in enumerate(dags, 2):
    dag_file = os.path.join(dags_dir, f"{dag_id}.py")
    content = DAG_TEMPLATE.format(dag_id=dag_id, description=description, layer=layer)
    with open(dag_file, 'w', encoding='utf-8') as f:
        f.write(content)
    print(f"✓ [{i:2d}/30] {dag_id}.py")
